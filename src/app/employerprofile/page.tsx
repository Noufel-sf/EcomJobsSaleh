...
const MOCK_JOBS: Job[] = [
	{
		...
		experience: "2+ years",
		salary: "$1,200 - $1,800",
		description: "Build and improve user-facing features with React and Next.js.",
		responsibilities: ["Build components", "Improve UX", "Collaborate with backend"],
...
export default function EmployerProfileViewPage() {
	const { data: profileResponse, isLoading: isCompanyLoading } = useGetProfileQuery();
	const { data: jobsResponse, isLoading: areJobsLoading } = useGetAllJobsQuery();

	const companyFromApi = (profileResponse?.user ?? null) as CompanyProfile | null;
	const company = companyFromApi ?? (USE_MOCK_DATA ? MOCK_COMPANY : null);

	const companyJobs = useMemo(() => {
		const jobs = jobsResponse?.content?.length ? jobsResponse.content : USE_MOCK_DATA ? MOCK_JOBS : [];
	...
	const specialization = company.specialization || company.industry || "Not specified";
			...
			<section className="mt-8">
				...
				{areJobsLoading && !USE_MOCK_DATA ? (
					...
					<div className="grid gap-4 md:grid-cols-2">
						{companyJobs.map((job) => (
							<article key={job.id} className="h-full rounded-xl border bg-card p-6">
