...
const sellersCopy: Record<Language, Record<string, string>> = {
  ...
  fr: {
    breadcrumb: "Vendeurs",
    title: "Vendeurs",
    description: "Gerez tous les vendeurs inscrits.",
    ...
    columns: "Colonnes",
    deleteUser: "Supprimer le vendeur",
  ...
  ar: {
    breadcrumb: "البائعون",
    title: "البائعون",
    description: "ادارة جميع البائعين المسجلين.",
...
export default function SuperAdminSellers() {
  ...
  const copy = sellersCopy[language];
  ...
  useEffect(() => {
    setTableData(sellers as Seller[]);
  }, [sellers]);

  const handleSuspend = async (user: Seller) => {
  ...
  const openDeleteDialog = (user: Seller) => {
  ...
  const handleVisitProfile = (user: Seller) => {
  ...
  const columns = [
    {
      id: "select",
      header: ({ table }: { table: TanstackTable<Seller> }) => (
        <Checkbox
